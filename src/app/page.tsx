...
export default function ClientesPage() {
  ...
  const [metrics, setMetrics] = useState<Metrics>({ total_jobs: 0, sucessos: 0, falhas: 0, por_tipo_arquivo: {}, por_tipo_erro: {} });
  ...
  useEffect(() => { fetchClients(); fetchMetrics(); }, []);

  const fetchClients = async () => {
    setLoading(true);
    try { const res = await fetch(`${API_BASE}/api/clientes`, { headers: { "X-API-Key": API_KEY } }); setClientes(await res.json()); }
    catch (err) { console.error(err); }
    finally { setLoading(false); }
  };

  const fetchMetrics = async () => {
    try { const res = await fetch(`${API_BASE}/api/clientes/metrics`, { headers: { "X-API-Key": API_KEY } }); setMetrics(await res.json()); }
    catch (err) { console.error(err); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Deseja excluir?")) return;
    await fetch(`${API_BASE}/api/clientes/${id}`, { method: "DELETE", headers: { "X-API-Key": API_KEY } });
  ...
  const handleOpenModal = (c?: any) => {
    if (c) { setModalData(c); setNome(c.nome); setOpenaiKey(c.openai_key); setApiKey(c.api_key); }
    else { setModalData({}); setNome(""); setOpenaiKey(""); setApiKey(""); }
    setOpen(true);
  };

  const handleSubmit = async () => {
    const method = modalData.id ? "PATCH" : "POST";
    const url = modalData.id ? `${API_BASE}/api/clientes/${modalData.id}` : `${API_BASE}/api/clientes`;
    await fetch(url, { method, headers: { "Content-Type": "application/json", "X-API-Key": API_KEY }, body: JSON.stringify({ nome, openai_key: openaiKey, api_key }) });
    setOpen(false); fetchClients(); fetchMetrics();
  };

  // Filtering
  const filtered = clientes.filter(c => {
    const matchesSearch = c.nome.toLowerCase().includes(search.toLowerCase());
    const matchesStatus = statusFilter === "all" || (statusFilter === "active" && c.ativo) || (statusFilter === "inactive" && !c.ativo);
  ...
  const fileData = Object.entries(metrics.por_tipo_arquivo).map(([n,v])=>({ name:n, value:v }));
  const errorData = Object.entries(metrics.por_tipo_erro).map(([n,v])=>({ name:n, value:v }));
    ...
    <div className="p-6 bg-slate-50 min-h-screen text-slate-900">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        ...
        <div className="flex gap-2">
          <Input placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)} className="bg-slate-100 placeholder-slate-400" />
          ...
          <Button variant="outline" onClick={() => { fetchClients(); fetchMetrics(); }}>
            <RefreshCw size={18} />
          </Button>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{modalData.id ? "Editar Instância" : "Nova Instância"}</DialogTitle>
                <DialogDescription>
                  {modalData.id ? "Atualize os dados da instância" : "Preencha os campos para criar uma nova instância"}
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <label className="text-right">Nome</label>
                  <Input className="col-span-3" value={nome} onChange={(e) => setNome(e.target.value)} />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <label className="text-right">OpenAI Key</label>
                  <Input type="password" className="col-span-3" value={openaiKey} onChange={(e) => setOpenaiKey(e.target.value)} />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          paged.map(c => (
            <Card key={c.id} className="bg-white border border-slate-200 shadow-sm">
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <label className="block text-xs text-slate-500">ID</label>
                    <span className="font-mono text-sm text-slate-700 truncate">{c.id}</span>
                  </div>
                  <div className="flex space-x-2">
                    <Button variant="ghost" size="icon" title="Copiar API Key" onClick={() => navigator.clipboard.writeText(c.api_key)}>
                      <Copy size={16} />
                    </Button>
                    <Button variant="ghost" size="icon" title="Resetar API Key" onClick={() => {
                      const newKey = crypto.randomUUID();
                    ...
                    <Button variant="ghost" size="icon" title="Editar" onClick={() => { /* open modal omitted */ }}>
                      <Edit2 size={16} />
                    </Button>
                    <Button variant="ghost" size="icon" title="Excluir" onClick={() => {
                      if (confirm('Deseja excluir?')) {
                        fetch(`${API_BASE}/api/clientes/${c.id}`, { method: 'DELETE', headers: { 'X-API-Key': API_KEY } }).then(() => { fetchClients(); fetchMetrics(); });
                      }
                    }}>
                      <Trash2 size={16} />
                    </Button>
                ...
                <div className="grid grid-cols-5 gap-4 mb-2 text-sm text-slate-700">
                  <div>
                    ...
                    <p className="text-base font-medium text-slate-900 truncate">{c.nome}</p>
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500">OpenAI Key</label>
                    <p className="truncate">{maskedKey(c.openai_key)}</p>
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500">Uso 30d</label>
                    <p className="font-semibold">${(c.uso * 0.01).toFixed(2)}</p>
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500">Atual</label>
                    <p className="font-semibold">${(c.uso_atual * 0.01).toFixed(2)}</p>
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500">Anterior</label>
                    <p className="font-semibold">${(c.uso_anterior * 0.01).toFixed(2)}</p>
                  </div>
                </div>
